...
def train(epoch, lr, list_stable, list_unstable, list_feature, list_adjacent, list_affine, list_epoch):
    ...
    for i in range(batch_idxs):
        ...
        if i%100==0:
            writer.add_image('image/unstable',vutils.make_grid(image_unstable2[5:8, period + 1:period + 1 + 3, :, :], normalize=True,scale_each=True),i + epoch * batch_idxs)
            if opt.use_gan and with_gan:
                writer.add_image('image/fake1_gray',vutils.make_grid(fake2_gray[5:8,:,:,:], normalize=True,scale_each=True),i + epoch * batch_idxs)
                writer.add_image('image/fake1_gray_crop',vutils.make_grid(fake_ab2[:,5:8,:,:].permute(1,0,2,3), normalize=True,scale_each=True),i + epoch * batch_idxs)
